...
func (l *Logger) logWithCaller(level logrus.Level, msg string) {
	colors := []string{
		...
		logrus.DebugLevel: "\033[35m", // Голубой для отладки
	}
...
func (l * Logger) Debug(msg string) { l.logWithCaller(logrus.DebugLevel, msg) }
...
func (l *Logger) Debugf(format string, args ...interface{}) {
	l.logWithCaller(logrus.DebugLevel, fmt.Sprintf(format, args...))
}